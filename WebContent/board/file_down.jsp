...
<body>
	<h1>WebContetn/fileupload/file_down.jsp</h1>
	<%
		...
		System.out.println("upload 폴더의 실제 주소(서버안에 있는 실제 주소) : " + sDownloadPath);
		...
		// => 스트림으로 데이터를 주고 받기 때문 => 해당 파일 웹에서 표현하는 방식 MIME타입
		...
		// * MIME 타입 : 클라이언트에게 전송된 문서의 다양성을 표현하기 위한 매커니즘
		...
		// * 사용자의 브라우저를 확인(IE 여부)
		// ie 다운로드시 한글파일의 이름이 깨짐, 공백문자가 "+"
		
		// 헤더에서 사용정보를 가져오기
		String agent = request.getHeader("User-Agent");
		System.out.println("사용자 정보 : " + agent);
		
		// 사용자 정보안에 "MSIE", "Trident" 둘중에 하나라도 있을 경우 참 => ie 이다.
		...
		// => 브라우저에서 해석되는 파일도 다운로드 처럼 처리하기위한 설정
		// "Content-Disposition", "attachment;"
		response.setHeader("Content-Disposition", "attachment; filename = " + fileName);
		
		
		// 다운로드
		// 브라우저를 사용해서 밖으로 출력 할수 있는 통로 생성
